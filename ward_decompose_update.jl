...
function ward_decompose(
    ...
    internal_buses_set = intersect(buses_set, internal_buses_set)
    internal_buses = sort(collect(internal_buses_set))
    ...
    #     end
    # end)
    # boundary_buses = sort(unique(boundary_buses))      
    internal_buses_set = Set(internal_buses)
    boundary_buses_set = Set()
    for b in branches
        from_in_internal = in(b.arc.from.number, internal_buses_set)
        to_in_internal = in(b.arc.to.number, internal_buses_set)

        if !from_in_internal && to_in_internal
            push!(boundary_buses_set, b.arc.from.number)
        end
        if from_in_internal && !to_in_internal
            push!(boundary_buses_set, b.arc.to.number)
        end
    end
    boundary_buses = sort(collect(boundary_buses_set))                                   #boundary buses   
